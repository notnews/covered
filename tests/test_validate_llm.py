...
def test_stratified_sample_respects_per_stratum() -> None:
    sample = validate_llm.stratified_sample(
        _frame(), strata=["era_id"], per_stratum=4, seed=1
    )
...
def test_stratified_sample_is_deterministic() -> None:
    a = validate_llm.stratified_sample(
        _frame(), strata=["era_id"], per_stratum=3, seed=42
    )
    b = validate_llm.stratified_sample(
        _frame(), strata=["era_id"], per_stratum=3, seed=42
    )
...
def test_stratified_sample_caps_at_stratum_size() -> None:
    ...
    sample = validate_llm.stratified_sample(
        small, strata=["era_id"], per_stratum=5, seed=0
    )
    assert len(sample) == 1
